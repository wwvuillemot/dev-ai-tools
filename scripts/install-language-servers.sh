...
# macOS: Homebrew LLVM installs clangd outside default PATH
if [[ "$OS" == "macos" ]]; then
  BREW_PREFIX="$(brew --prefix 2>/dev/null || echo "/opt/homebrew")"
  export PATH="$BREW_PREFIX/opt/llvm/bin:$PATH"
fi

# Use sudo for apt-get when not running as root
APT_GET="apt-get"
...
LANG_DEFS=(
  ...
  "cpp        | C / C++ (clangd)            | CMakeLists.txt *.cpp *.cc *.c *.h   | clangd                                    |         | brew install llvm                                                                                   | $APT_GET install -y clangd                                                                            | Add compile_commands.json to project root"
  ...
  "kotlin     | Kotlin (kotlin-lsp)         | *.kt *.kts                          | kotlin                                    | kotlin  | brew install kotlin                                                                                 | $APT_GET install -y kotlin                                                                            | kotlin-lsp is pre-alpha"
  ...
  "erlang     | Erlang (erlang_ls)          | rebar.config *.erl                  | erlang_ls                                 |         | brew install erlang-ls                                                                              | $APT_GET install -y erlang-ls                                                                         | Requires Erlang/OTP"
  ...
  "zig        | Zig (ZLS)                   | *.zig build.zig                     | zls                                       |         | brew install zls                                                                                    | $APT_GET install -y zls || snap install zls --classic                                                 | ZLS version must match Zig version"
